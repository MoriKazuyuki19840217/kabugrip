...
if submitted and ticker:
    try:
        ...
        st.success(f"✅ {ticker} を追加しました！（保有 {holding_days}日）")
        
    except Exception:
        st.error(f"❌ {ticker} のデータ取得に失敗しました。正しいコードか確認してください（BTC-USD / ^GSPC など）")
...
if not st.session_state.portfolio.empty:
    ...
    # 握力計算ロジック（保有期間が長いほど強く、含み損が小さいほど強い）
    df["握力貢献"] = (df["保有期間(日)"] / 30) + ((100 - df["含み損率(%)"].fillna(0).abs()) / 4)
    total_score = df["握力貢献"].sum() + (len(df) * 12)
    grip = max(10, min(100, int(total_score)))
    
    st.subheader("📊 あなたの総合握力")
    st.progress(grip / 100.0)
    st.metric("握力", f"{grip} kg 💪")
    
    if grip >= 80:
        st.success("最強クラス！ グローバル分散の握力の達人です🔥")
    elif grip >= 60:
        st.info("かなり強い握力！ 仮想通貨やS&P500も加えてさらに鍛えよう")
    elif grip >= 40:
        st.warning("平均以上。保有期間を伸ばすと握力が上がります")
    else:
        st.error("握力強化トレーニング中… 小さく始めて長期保有を習慣に！")
    
    # グラフ
    st.bar_chart(df.set_index("銘柄コード")["保有期間(日)"], color="#00ccff")
else:
    st.info("まだ銘柄がありません。上記のフォームで追加してみてください！\n（S&P500・BTC・ETHなどもワンタップで追加できます）")

st.caption("※ 価格は yfinance の参考値です。実際の取引は自己責任で。")# ====================== 入力フォーム ======================
    with st.form("add_stock"):
        col1, col2 = st.columns([2, 1])
...
st.caption("※ 価格は yfinance の参考値です。実際の取引は自己責任で。")
